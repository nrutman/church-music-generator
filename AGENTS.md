...
- `generated/` — Generated `.docx` chord sheets and lyric sheets (git-ignored)
- `src/` — Generator scripts and code
- `src/songs/` — Song data files (JSON) — one per song, kept for re-generation and debugging (git-ignored)
- `node_modules/` — Dependencies (docx npm package)

Keep generated documents in `generated/` and code in `src/`.
