...
1. Before the first actual publish in a batch, create a timestamped backup under `~/Downloads/Table-Based Song Sheets/Backups/`.
2. Back up every matching Drive `.doc`/`.docx` that may be replaced.
3. Download each existing Planning Center Word attachment through its attachment `open` action.
...
6. Report the backup path, published songs, placements, and verification counts.
